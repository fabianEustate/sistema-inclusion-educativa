...
import noticiasService from "@/services/noticias/noticias_service";
 ...
 useEffect(() => {
  ...
  const cargarNoticias = async () => {
    try {
      ...
      const respuesta = await noticiasService.getNoticias();

      // 👈 EL BACKEND YA DEVUELVE UN ARRAY DIRECTAMENTE
      const noticiasAPI = respuesta || [];

      console.log("Noticias desde API:", noticiasAPI);

      // 🔥 Mapeo con el modelo del backend
      const noticiasMapeadas = noticiasAPI.map(noticia => ({
        id: noticia.id_noticia,
        titulo: noticia.titulo,
        subtitulo: noticia.subtitulo,
        lead: noticia.lead,
        cuerpo: noticia.cuerpo,
        categoria: noticia.categoria?.toLowerCase(),
        ubicacion: noticia.ubicacion?.toLowerCase(),
        estado: noticia.estado?.toLowerCase(),
        fecha_publicacion: noticia.fecha_publicacion,
        fecha_registro: noticia.fecha_registro ?? noticia.fecha_publicacion,
        activo: noticia.activo,
        id_usuario: noticia.id_usuario,
        autor: noticia.autor ?? "Administrador"
      }));

      const ordenadas = noticiasMapeadas.sort(
        (a, b) => new Date(b.fecha_publicacion) - new Date(a.fecha_publicacion)
      );

      setNoticias(ordenadas);
      setNoticiasFiltradas(ordenadas);
  ...
  const eliminarNoticia = async (id) => {
    const confirmar = confirm("¿Seguro deseas eliminar esta noticia?");
    if (!confirmar) return;

    try {
      await noticiasService.eliminarNoticia(id);

      // Quitarla del estado local sin recargar
      setNoticias(prev => prev.filter(n => n.id !== id));
      setNoticiasFiltradas(prev => prev.filter(n => n.id !== id));
      ...
      <div className="col-12 md:col-6 lg:col-4">
        <Card className={styles.noticiaCard}>
          ...
          <div className={styles.cardContent}>
            <p className={styles.noticiaSubtitulo}>{noticia.subtitulo || 'Sin subtítulo'}</p>
